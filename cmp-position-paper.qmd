...
CMP does not depend on AI-assisted development. Human developers have always paid context cost through attention, navigation, working memory, review time, and missed assumptions. What AI-assisted workflows change is not the existence of this cost, but its observability.

An AI coding agent must construct an explicit working context before acting: files are retrieved, prompts are assembled, tools are called, tests are run, and failures are surfaced. These workflows expose signals that were previously difficult to measure directly: prompt size, retrieval breadth, number of files inspected, tool-call count, localization accuracy, retry count, patch size, and test outcomes. A codebase that forces an agent to retrieve many unrelated files or repeatedly chase hidden conventions is making context cost visible in a mechanical form.

This does not mean software should be designed only for agents. Humans and agents use context differently: humans rely on memory, visual scanning, social knowledge, and domain intuition, while agents rely more heavily on explicit artifacts available in the prompt or tool environment. But both benefit from the same structural properties: reliable boundaries, localized domain decisions, explicit contracts, and tests that encode behavior rather than implementation accident.

AI-assisted development therefore gives CMP a practical research opportunity. It turns context minimization from a largely qualitative design intuition into something that can be studied through observable traces. The research question is not whether agents replace human judgment, but whether context-oriented design metrics predict better outcomes for both human and agent maintenance tasks.
...
CMP builds on information hiding, modularity, and data abstraction rather than replacing them. Parnas's hidden design decisions, abstract types, and Ousterhout's deep modules all describe ways to reduce the implementation detail a caller must load [@parnas1972criteria; @liskov1987data; @ousterhout2018philosophy]. CMP generalizes this intuition by treating information hiding as one instance of a broader context-control problem, and by pairing depth control with breadth control.

CMP is also close in spirit to local reasoning. Formal methods such as separation logic limit the heap context required for proof [@reynolds2002separation]. CMP operates at a different level: practical software comprehension, modification, review, and generation. Its concern is not only what can be proven locally, but what a human or agent must load to answer an engineering query reliably.

Finally, CMP differs from traditional software metrics in its target. Cyclomatic complexity, object-oriented metric suites, and cognitive complexity measure important proxies for complexity and maintainability [@mccabe1976complexity; @chidamber1994metrics; @campbell2018cognitive]. A context-oriented metric would instead ask what must be loaded, from where, and across which boundaries, for a particular query family. In this sense CMP is less a competing metric than a proposal for what future metrics should try to approximate.

## Conclusion

CMP proposes that software design be judged by the query-relevant context it forces a reader to load. Because common maintenance queries cluster around understanding behavior and changing purpose, context expands mainly as **depth** through dependency traversal and **breadth** through dispersed knowledge. This framing explains familiar design principles and language abstractions as context controls rather than isolated prescriptions.

The depth-breadth trade-off gives CMP its practical edge. Abstraction, layering, configuration, and duplication are not good or bad in isolation. They are justified only when they improve the depth-breadth profile for the queries a system actually faces. The **Over-engineering Criterion** names the failure case: a design move is suspect when the depth it introduces is not justified by the breadth it removes.

CMP remains a conceptual foundation rather than a finished measurement theory. The next step is empirical: recover realistic query distributions, estimate depth and breadth, and test whether context-oriented metrics predict better human and agent maintenance outcomes. AI-assisted workflows matter here not because CMP is agent-specific, but because they make context cost newly observable through retrieval, prompts, tool calls, retries, patches, and tests.
